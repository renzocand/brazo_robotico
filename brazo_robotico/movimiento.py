...
class Movimiento:
    ...
    def generar_secuencia(self) -> dict[str, Angulos]:
        ...
        # Validación
        if not self.sistema.es_alcanzable(inicio.x, inicio.y):
            raise ValueError(f"Posición inicial {self.casilla_inicio} no alcanzable")
        if not self.sistema.es_alcanzable(fin.x, fin.y):
            raise ValueError(f"Posición final {self.casilla_fin} no alcanzable")

        # Cálculo de ángulos (ahora 3)
        angulos_inicio = self.sistema.calcular_angulos(inicio.x, inicio.y)
        angulos_fin = self.sistema.calcular_angulos(fin.x, fin.y)
