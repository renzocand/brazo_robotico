...
class CinematicaInversa:
    def es_alcanzable(self, x: float, y: float, L1: float, L2: float) -> bool:
        """
        Devuelve True si el punto (x,y) está dentro del alcance radial del brazo.

        x representa el desplazamiento lateral y y la distancia frontal desde la base.
        """
    ...
    def calcular_angulos(self, x: float, y: float, L1: float, L2: float) -> Angulos:
        ...
        La base gira en el plano horizontal y los dos eslabones se resuelven
        sobre la distancia radial al objetivo.
        """
        theta_rot = 90.0 - degrees(atan2(x, y))
        distancia_radial = sqrt(x**2 + y**2)

        if not self.es_alcanzable(x, y, L1, L2):
            raise ValueError(f"Posición ({x},{y}) fuera de alcance")

        cos_theta2 = (distancia_radial**2 - L1**2 - L2**2) / (2 * L1 * L2)
